...
// Add Aave’s Lending Pool Contract and ABI
import LENDING_POOL_ABI from "./abis/lendingpool.json" assert { type: "json" };

const AAVE_LENDING_POOL_CONTRACT_ADDRESS =
  "0x7bD9e2a3c7F5A0bDde5ED5F02Cf7E3c3B93d9e0e"; // Update this with the correct contract address on the Sepolia network
...
async function main(swapAmount) {
  ...
  try {
    ...
    // Supply the obtained LINK to Aave
...
// Part G - Write Supply to Aave Function
async function supplyToAave(lendingPoolAddress, tokenAddress, amount, wallet) {
  try {
    ...
    const approveTransaction = await approveToken(
      tokenAddress,
      TOKEN_IN_ABI,
      amount,
      wallet
    ...
    const transactionResponse = await lendingPoolContract.deposit(
      tokenAddress,
      amount,
      wallet.address,
      0 // referralCode is typically 0
    );

    console.log(`-------------------------------`);
    console.log(`Sending Supply Transaction to Aave...`);
    console.log(`-------------------------------`);
    console.log(`Transaction Sent: ${transactionResponse.hash}`);
    console.log(`-------------------------------`);
    const receipt = await transactionResponse.wait();
...
// Enter USDC AMOUNT TO SWAP FOR LINK AND SUPPLY TO AAVE
main(1);
